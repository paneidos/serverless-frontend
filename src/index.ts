...
import {StandardCacheControl, StandardSiteBucket, StandardSiteBucketPolicy} from "./s3";
import {
...
interface FrontendConfig {
    ...
    cloudfront?: {
        ...
        ssl_version?: Exclude<CloudfrontDistributionConfig['ViewerCertificate'], undefined>['MinimumProtocolVersion']
    }
}
...
class FrontendPlugin implements Plugin {
    ...
    constructor(serverless: Serverless & ServerlessOutputs, options: Serverless.Options, { log, progress } : Plugin.Logging) {
        ...
        this.hooks = {
            "before:package:initialize": () => this.serverless.pluginManager.spawn('frontend:addFunctions'),
            "before:info:info": () => this.serverless.pluginManager.spawn('frontend:addFunctions'),
            "after:aws:info:displayEndpoints": this.addSiteUrl.bind(this),
            "before:package:finalize": this.addResources.bind(this),
            "before:package:createDeploymentArtifacts": () => this.serverless.pluginManager.spawn('frontend:build'),
            "before:package:function:package": () => this.serverless.pluginManager.spawn('frontend:build'),
            "after:deploy:deploy": () => this.serverless.pluginManager.spawn('frontend:upload'),
    ...
    async detectFramework() : Promise<Framework | null> {
        ...
        } else if ('vite' in packageJson.dependencies || 'vite' in packageJson.devDependencies) {
    ...
    async getStackOutputs(): Promise<Record<string, string>> {
        ...
        return result.Stacks[0].Outputs.reduce((obj: Record<string, string>, output: AwsOutput) => {
            return {
                ...obj,
    ...
    async addCloudfrontResources() {
        ...
        switch(framework) {
            ...
            case 'tanstack-start':
                ...
                distributionConfig.DefaultCacheBehavior = StandardCacheBehaviors.serverFunction
            ...
            case 'vite':
                ...
                distributionConfig.DefaultCacheBehavior = StandardCacheBehaviors.staticFilesWithFallback
                break;
        }
    ...
    async packageFunction(file: string, cwd: string, pattern : string|string[] = ['*', '**/*']) {
    ...
    async packageFunctions() {
        switch(await this.detectFramework()) {
            ...
            case 'nuxt':
            case 'nitro':
                await this.packageFunction('.serverless/frontend-function.zip', '.output');
        }
    }
    ...
    async uploadAssets() {
        ...
        const directory = framework === 'nitro' || framework === 'tanstack-start' || framework === 'nuxt' ? '.output/public' : 'dist'
        const immutableAssets = framework == 'nuxt' ? /^_nuxt\// : (
            framework === 'tanstack-start' ? /^assets\// : (
        ...
        for (const file of files) {
            ...
            const cacheControl = baseKey.match(immutableAssets) ? cacheControls.immutable : cacheControls.normal
            const params = {
    ...
    async createInvalidation() {
        ...
        } = await this.provider.request('CloudFormation', 'describeStackResources', { StackName: stackName });
